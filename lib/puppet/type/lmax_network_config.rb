...
module Puppet
  ...
  Puppet::Type.newtype(:lmax_network_config) do
 ...
 DNS{1,2} are set, or if using dhclient. default to 'yes'."
    ...
    newparam(:type) do
      ...
      newvalues(:Ethernet, :Bridge, :Bonding, :Wireless)
    end
    ...
    newparam(:ethtool_opts) do
      desc "Configures the ethtool on the device"
    end
    ...
    newparam(:mtu) do
      desc "Configures the MTU size on an interface"
    end

    newparam(:essid) do
      desc "Configures the ESSID on a wireless interface"
    end

    newparam(:key_mgmt) do
      desc "Configures the key_mgmt setting on a wireless interface"
    end

    newparam(:defroute) do
      desc "Configures the defroute setting on an interface"
      newvalues(:yes, :no)
    end

    newparam(:ipv4_failure_fatal) do
      desc "Configures the ipv4_failure_fatal setting on an interface"
      newvalues(:yes, :no)
    end

    newparam(:ipv6init) do
      desc "Configures the ipv6init setting on an interface"
      newvalues(:yes, :no)
    end

    newparam(:ipv6_autoconf) do
      desc "Configures the ipv6_autoconf setting on an interface"
      newvalues(:yes, :no)
    end

    newparam(:mode) do
      desc "Configures the mode setting on a wireless interface"
    end

    newparam(:securitymode) do
      desc "Configures the securitymode setting on a wireless interface"
    end

    newparam(:peerroutes) do
      desc "Configures the peerroutes setting on an interface"
      newvalues(:yes, :no)
    end
